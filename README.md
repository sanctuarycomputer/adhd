...
| `/adhd:push-tokens` | `[--dry-run] [--include-tailwind]` | code → Figma | Pushes globals.css variables + named styles into Figma directly via the remote MCP. Runs an interactive 7-question wizard on every invocation to set per-domain push policy (push all colors vs semantic-only, skip opacity entirely, route shadows through effect styles, etc.). `--dry-run` previews what would be added or skipped without writing — the preview reflects your wizard answers. `--include-tailwind` also surfaces every Tailwind v4 default in the diff (full palette, spacing scale, etc.) — use as a one-time seed so designers have every utility available as a Figma variable. |
