...
| `/adhd:push-tokens` | `[--dry-run] [--include-tailwind]` | code → Figma | Pushes globals.css variables + named styles into Figma directly via the remote MCP. `--dry-run` previews what would be added or overwritten without writing. `--include-tailwind` also pushes every Tailwind v4 default (full palette, spacing scale, etc.) — use as a one-time seed so designers have every utility available as a Figma variable. |
